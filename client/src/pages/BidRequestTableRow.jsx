    ...
    return (
        ...
        {format(new Date(deadline),'p')}
          ...
          <td className='px-4 py-4 text-sm font-medium text-gray-700 whitespace-nowrap'>
            <div   className={`inline-flex items-center px-3 py-1 rounded-full gap-x-2  ${status === 'Pending' && 'bg-yellow-100/60 text-yellow-500'} 
                        ${status === 'In progress' && 'bg-green-100/60 text-green-500'}
                        ${status === 'Completed' && 'bg-blue-100/60 text-blue-500'}
                        ${status === 'Rejected' && 'bg-green-100/60 text-red-500'}`}>
              <span  className={`h-1.5 w-1.5 rounded-full ${status === 'Pending' && 'bg-yellow-500'} 
              ${status === 'Completed' && 'bg-blue-500'}
                          ${status === 'In progress' && 'bg-green-500'}
                          ${status === 'Rejected' && 'bg-red-500'}`}></span>
